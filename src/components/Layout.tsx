...
const myPlanMenu = [
	{ title: "Cardápio Semanal", url: "/cardapio", icon: UtensilsCrossed },
...
const intelligentMenu = [
	{ title: "Assistente Nutricional", url: "/chat-nutri", icon: UtensilsCrossed },
	{ title: "Análise de Progresso IA", url: "/progresso-ia", icon: Activity },
	{ title: "Check-in Diário", url: "/checkin-diario", icon: Calendar },
	{ title: "Ajustes Automáticos", url: "/ajustes-automaticos", icon: Settings },
	{ title: "Menu Restaurante", url: "/menu-restaurante", icon: Camera },
	{ title: "Relatórios de Evolução", url: "/relatorios-evolucao", icon: FileText },
];

const libraryMenu = [
	{ title: "Receitas por Morbidade", url: "/ebooks", icon: BookOpen },
	{ title: "Guias Alimentares Científicos", url: "/guias-cientificos", icon: BookOpen },
	{ title: "Protocolos Nutricionais", url: "/protocolos-nutricionais", icon: Activity },
];

const accountMenu = [
	{ title: "Perfil", url: "/perfil", icon: User },
	{ title: "Preferências Nutricionais", url: "/preferencias-nutricionais", icon: Settings },
...
function AppSidebar() {
	...
	const handleLogout = () => {
		...
		document.cookie.split(";").forEach(cookie => {
			...
			if (name.includes("dietcase") || name.includes("firebase") || name.includes("auth")) {
		...
		console.log("🧹 Todos os dados de autenticação foram limpos - Usuário precisará fazer login novamente");
	...
	const isTransformacao =
		currentUser.plan === "TRANSFORMACAO"
	...
	return (
		<Sidebar collapsible="icon">
			<SidebarContent>
				...
				<SidebarGroup>
					<SidebarGroupLabel className="text-sidebar-foreground/70">
						{open ? "Acompanhamento Inteligente" : ""}
					</SidebarGroupLabel>
					<SidebarGroupContent>
						<SidebarMenu>
							{intelligentMenu.map((item) => {
								const isLocked = !isTransformacao;
								return (
									<SidebarMenuItem key={item.title}>
										<SidebarMenuButton
											asChild
										...
										>
											<NavLink
												to={item.url}
												onClick={handleVisitorClick}
												className="flex items-center gap-2 hover:bg-sidebar-accent text-sidebar-foreground transition-colors"
												activeClassName="bg-sidebar-accent text-sidebar-accent-foreground font-medium"
											>
				...
				<header className="sticky top-0 z-50 flex h-16 w-full shrink-0 items-center gap-2 border-b bg-background/95 backdrop-blur-md px-4 sm:px-8 transition-all ease-linear shadow-sm">
					<div className="flex items-center gap-2">
						...
						<div className="flex items-center gap-2 overflow-hidden shrink-0">
							...
							/>
							<span className="font-bold text-lg truncate min-w-[80px]">DietCase</span>
						</div>
					</div>
