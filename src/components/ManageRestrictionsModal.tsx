...
const restrictionTypeLabels: Record<UserRestriction['type'], { label: string; color: string }> = {
...
export function ManageRestrictionsModal({ open, onOpenChange }: ManageRestrictionsModalProps) {
  const { restrictions, addRestriction, removeRestriction } = useRestrictions();
  const [newIngredient, setNewIngredient] = useState('');
  const [selectedType, setSelectedType] = useState<UserRestriction['type']>('nao-gosto');
  ...
  const handleAdd = () => {
    ...
    addRestriction(newIngredient.trim(), selectedType);
    setNewIngredient('');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Adicione ingredientes que você não pode ou não quer consumir. O sistema sugerirá substituições automaticamente.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 mt-4">
          ...
          <div className="space-y-4 p-4 border rounded-lg bg-muted/50">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select value={selectedType} onValueChange={(value) => setSelectedType(value as UserRestriction['type'])}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="restricao">⚕️ Restrição Médica</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>

            <Button onClick={handleAdd} className="w-full inline-flex items-center justify-center gap-2">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium capitalize">{restriction.ingredient}</p>
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => handleRemove(restriction.ingredient)}
                      >
                        <X className="h-4 w-4" />
                      </Button>
                    </div>
          ...
          <div className="p-4 bg-muted rounded-lg border">
            <p className="text-sm">
              <strong>💡 Dica:</strong> Após adicionar suas restrições, o sistema automaticamente sugerirá 
              substituições inteligentes nas refeições que contenham esses ingredientes, mantendo valores 
