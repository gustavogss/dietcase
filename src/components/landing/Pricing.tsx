...
export function Pricing() {
  ...
  const goToPlanDashboard = (plan: PlanType) => {
    const key = 'dietcase-user-profile';
  ...
  return (
    <section ref={sectionRef} id="pricing" className="section-padding bg-muted/30">
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6">
        <div data-reveal className="section-heading">
          <h2 className="text-foreground">Planos e Preços</h2>
          <p className="text-muted-foreground font-medium">Escolha o plano ideal para você</p>
        </div>

        <div data-stagger className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 max-w-4xl mx-auto justify-items-center">
          {landingData.plans.map((plan) => (
            <Card
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' || e.key === ' ') goToPlanDashboard(plan.name as PlanType);
              }}
            >
              {plan.highlight && (
                <div className="absolute -top-4 left-1/2 -translate-x-1/2">
                  <Badge className="px-6 py-1 font-bold shadow-lg">Mais Popular</Badge>
                </div>
              )}

              <CardHeader className="card-padding pb-3">
                <CardTitle className="text-fluid-xl font-bold mb-3">{plan.name}</CardTitle>
                <div className="flex items-baseline gap-1">
                  <span className="text-fluid-4xl font-black text-foreground tracking-tighter">R$ {plan.price}</span>
                ...
                <ul className="space-y-3">
                  {plan.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      ...
                      <span className="text-fluid-sm text-muted-foreground font-medium">{feature}</span>
                    </li>
                  ))}
                </ul>
              </CardContent>
