...
export default function Profile() {
  ...
  const handleSaveProfile = async (updatedData: Partial<UserProfile>) => {
    ...
    if (authUserId) {
      await userService.updatePersonalization(updatedData);
    }
  };
  ...
  const handleSaveMorbidities = async (morbidityIds: string[]) => {
    ...
    if (authUserId) {
      await userService.updatePersonalization({ morbidities: morbidityIds });
    }
  };
  ...
  const handleUpgradePlan = async (newPlan: PlanType) => {
    ...
    if (authUserId) {
      await userService.updatePlan(newPlan);
    }
  };
