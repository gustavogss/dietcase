...
export default function ShoppingList() {
  ...
  const loadShoppingList = async () => {
    ...
    try {
      const persistedPlan = currentUser.id
        ? await nutritionalPlanService.getUserLatestPlan(currentUser.id)
        : null;
      const generatedList = persistedPlan
        ? generateShoppingListFromMenu(persistedPlan.menu)
        : [];
      setCategories(generatedList);
      setHasPersistedPlan(Boolean(persistedPlan));
      setSelectedCategory(generatedList[0]?.id ?? "");
      setPlanLoadError("");
    } catch (error) {
      console.error("Erro ao carregar plano persistido do Firestore", error);
      setCategories([]);
      setHasPersistedPlan(false);
      setPlanLoadError("Não foi possível carregar seu plano alimentar agora.");
    } finally {
  ...
  const handleRefreshList = () => {
    ...
    toast({
      ...
      description: hasPersistedPlan
        ? `Sincronizada para: ${formattedMorbidities || "Geral"}`
        : "Nenhum plano alimentar persistido foi encontrado.",
    });
  };
    ...
    <div className="container mx-auto max-w-screen-xl px-4 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-6 font-sans">
      ...
      <div className="grid gap-6 md:grid-cols-[2fr_1fr]">
        <div className="space-y-6">
          ...
          {categories.length === 0 && !isLoadingPlan && (
            <Card className="flex flex-col items-center justify-center p-12 text-center text-muted-foreground">
              ...
              <p className="text-sm">
                {!planLoadError && !hasPersistedPlan
                  ? "Gere e salve um plano alimentar para criar sua lista de compras."
                  : "Os itens do seu plano alimentar aparecerão aqui."}
              </p>
            </Card>
          )}
        </div>
