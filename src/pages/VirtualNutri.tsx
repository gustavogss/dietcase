...
import { virtualNutriChatAgent } from "@/services/agents";
...
export default function VirtualNutri() {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const agentResult = await virtualNutriChatAgent.process({
        conversationId: convId,
        message: trimmed,
        userContext: {
  ...
  const handleUpgradePlan = (newPlan: PlanType) => {
    persistUserProfile({ ...profile, plan: newPlan });
