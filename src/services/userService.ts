...
class UserService {
  ...
  async createUser(userData: Omit<FirebaseUser, 'createdAt'>): Promise<void> {
    ...
    const userDoc: Omit<FirebaseUser, 'createdAt'> & { createdAt: any } = {
      ...userData,
  ...
  async updatePlan(plan: PlanType): Promise<void> {
    ...
    await updateDoc(doc(db, this.usersCollection, auth.currentUser.uid), updateData);
  }
  ...
  async createUserWithTrial(userData: Omit<FirebaseUser, 'plan' | 'createdAt' | 'trialStartDate' | 'trialExpiresAt'>): Promise<void> {
    const now = new Date();
    const trialExpires = new Date(now.getTime() + (7 * 24 * 60 * 60 * 1000)); // +7 dias
  ...
  async updateUser(userData: Partial<Omit<FirebaseUser, 'createdAt' | 'plan'>>): Promise<void> {
    if (!auth.currentUser) {
      throw new Error("Usuário não autenticado");
    }

    await updateDoc(doc(db, this.usersCollection, auth.currentUser.uid), userData);
  }
}
